...
# 3. TESTING ( Lookup table for equivalent ratings)
RATING_MAP = {
    'it was amazing': 5, 
    'really liked it': 4, 
    'liked it': 3,
...
# 4. VISUAL PEEK FUNCTION (Explicitly for both Books and Ratings)
def peek_at_data(file_path, num_rows=5):
    ...
    try:
        ...
        logging.error(f"Error peeking at {file_name}: {e}")
...
def run_etl():
    ...
    if os.path.exists(target_book): peek_at_data(target_book)
    if os.path.exists(target_rating): peek_at_data(target_rating)
