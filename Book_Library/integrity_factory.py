...
logging.info("--- INTERGRITY FACTORY STARTING---")

#The database connection
connection = sqlite3.connect('integrity_v2.db')
...
# TEST 1: The Duplicate (This will be ignored by the database)
    cursor.execute('''
        INSERT OR IGNORE INTO books (title, author_id)
        VALUES (?, ?)
    ''', ('The Hobbit', author_id))

# TEST 2: The New Entry (This will be added successfully)
    cursor.execute('''
        INSERT OR IGNORE INTO books (title, author_id)
        VALUES (?, ?)
    ...
    connection.commit()
    logging.info('''
             SUCCESS: Integrity Check Passed & Database Updated
''')
